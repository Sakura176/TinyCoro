...
def run_test(
    ...
    try:
        ...
        if bench_process.poll() is None:  # running
            ...
            print(
                f"{RED}[error] {
                    benchtool_path} hasn't finish running, please check your server code{RESET}"
            )
            bench_process.kill()
            if program_process.poll() is None:
                program_process.kill()
...
if __name__ == "__main__":
    for i, para in enumerate(paras):
        ...
        print(
            f"{GREEN}thread number: {
                thread_num}, server port: {para[1]}{RESET}"
        )
        if not run_test(para[0], para[1], sys.argv[1], sys.argv[2]):
            print(
                f"{RED}xxxxxxxxxxxxxxxxxx lab3 test case {
                    i} error xxxxxxxxxxxxxxxxxx{RESET}"
            )
            sys.exit(1)
        else:
            print(
